...
const LESSON = {
  ...
  missions: {
    mission1: {
      title: "怎麼來",
      copy: ["瑞容想先打電話問問大家的交通方式。", "請選指令、選朋友，接著用詞卡排出問句。"],
      commands: ["打給", "前往"],
      sentence: "你愛仰仔來吾屋下？",
...
function resetStageState() {
  ...
  if (STAGES[state.stageIndex].id === "mission1") state.completedMission1 = [];
}
    ...
    <div class="panel mission-panel">
      <div class="mission-top">
        ...
        <div class="mission-commands">
          ${mission.commands.map((command) => `<button class="mission-command ${state.selectedCommand === command ? "is-selected" : ""}" type="button" data-command="${command}">${command}</button>`).join("")}
        </div>
      </div>
...
function missionSideContent(stage, mission) {
  if (stage === "mission1" && state.selectedCommand === "打給" && state.selectedPerson && state.askedPerson !== state.selectedPerson && !state.completedMission1.includes(state.selectedPerson)) {
    return sentencePanel(mission);
  }
  if (stage === "mission1" && state.askedPerson && !state.completedMission1.includes(state.askedPerson)) {
    if (state.selectedCommand === "前往" && state.travelPerson) return transportPanel(mission);
    return responseAudioPanel(mission);
  }
  return `
    <div class="mission-notes">
      <strong>${stage === "mission1" ? "任務 1 可完整通關" : "待接任務完整判定"}</strong><br>
      目標句：${mission.sentence}<br><br>
      ${mission.answers.map((answer) => `<span class="answer-chip">${answer}</span>`).join("<br>")}
      ${stage === "mission1" ? completedList() : ""}
    </div>
  `;
}
...
function completedList() {
  if (!state.completedMission1.length) return "";
  return `<br><br><strong>已完成</strong><br>${state.completedMission1.map((name) => `<span class="answer-chip">${name}</span>`).join("")}`;
}
...
function bindMissionEvents(stage, mission) {
  ...
  document.querySelectorAll("[data-person]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      playSe(4);
      if (state.selectedCommand === "前往") {
        if (!state.askedPerson) {
          flashHint("請先用「打給」問一位朋友。");
          return;
        }
        if (button.dataset.person !== state.askedPerson) {
          flashHint(`剛剛問的是${state.askedPerson}，請選同一位朋友。`);
          return;
        }
        state.travelPerson = button.dataset.person;
        state.transportResult = "";
        renderMission(stage);
        return;
      }
      if (state.selectedCommand !== "打給") {
        flashHint("請先選「打給」，再選朋友。");
        return;
      }
  ...
  bindOptional("#submitSentence", "click", () => {
    ...
    if (answer === target) {
      ...
      state.sentenceResult = `答對了！請按${state.selectedPerson}的音檔聽交通方式。`;
      ...
      playSe(7);
      state.sentenceResult = "還差一點，請調整詞卡順序。";
    }
    renderMission(stage);
  });
  bindOptional("#hintSentence", "click", () => {
    state.answerTokens = ["你", "愛", "仰仔", "來", "吾屋下"];
    state.sentenceResult = "已放入提示詞卡，可以送出看看。";
    renderMission(stage);
  });
  ...
  document.querySelectorAll("[data-transport]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const expected = mission.transportByPerson[state.travelPerson];
      if (transport === expected) {
        playSe(9);
        state.completedMission1.push(state.travelPerson);
        state.transportResult = `${state.travelPerson}會${transport}來，完成！`;
        state.selectedCommand = "";
        state.selectedPerson = "";
        state.askedPerson = "";
        state.travelPerson = "";
        ...
        playSe(7);
        state.transportResult = `不是${transport}，再想想看。`;
      }
      renderMission(stage);
    });
  });
...
function missionHint(stage) {
  if (!state.selectedCommand) return "請選取指令積木";
  if (stage === "mission1" && state.selectedCommand === "打給" && !state.selectedPerson) return "打給 + [請選取你要打電話給誰]";
  if (stage === "mission1" && state.selectedCommand === "打給" && state.selectedPerson) return `打給 + ${state.selectedPerson} + [請重組句子並發問]`;
  if (stage === "mission1" && state.questionPlaying) return "問句播放中，播完後會出現回答音檔按鈕";
  if (stage === "mission1" && state.askedPerson && !state.responseReady) return "請稍等問句播放完成";
  if (stage === "mission1" && state.askedPerson && !state.selectedCommand) return `請先聽${state.askedPerson}的回答音檔，再選「前往」`;
  if (stage === "mission1" && state.selectedCommand === "前往" && state.askedPerson && !state.travelPerson) return `前往 + [請選取${state.askedPerson}]`;
  if (stage === "mission1" && state.selectedCommand === "前往" && state.travelPerson) return `前往 + ${state.travelPerson} + [請選擇交通方式]`;
  return `已選取「${state.selectedCommand}」，下一步會接完整任務判定。`;
}

function sentencePanel(mission) {
  const tokens = mission.tokens.map((token) => {
    const disabled = state.answerTokens.includes(token) ? "disabled" : "";
    return `<button class="token-chip" type="button" data-token="${token}" ${disabled}>${token}</button>`;
  }).join("");
  const answer = state.answerTokens.length
  ...
  return `
    <div class="sentence-panel">
      <div class="sentence-head">
        <strong>字句重組</strong>
        <button class="icon-button" type="button" id="playAskVoice">播放問句</button>
      </div>
      <div class="answer-zone">${answer}</div>
      <div class="token-bank">${tokens}</div>
      <div class="sentence-actions">
        <button class="icon-button" type="button" id="submitSentence">送出</button>
        <button class="icon-button" type="button" id="clearSentence">清空</button>
        <button class="icon-button" type="button" id="hintSentence">提示</button>
      </div>
      <div class="sentence-result">${state.sentenceResult}</div>
    </div>
  `;
}
...
function responseAudioPanel(mission) {
  ...
  const text = state.responseReady
    ? `按播放音檔，聽${state.askedPerson}要搭什麼交通工具。聽完後，請按「前往」，再點${state.askedPerson}。`
    : "系統正在播放你剛剛拼出的問句。";
  return `
